...
export async function loadPublicaciones(opts: LoadOpts = {}): Promise<ProductCardData[]> {
  try {
    ...
    let query = sb
      ...
      .select(
        'producto_id, slug, titulo_web, precio_oferta, etiquetas, productos!inner(id, nombre, imagen_principal_url, categoria_id, campana_id, productos_variantes(precio_publico))',
      )
      .eq('publicado', true)
      .order('orden_web')
      .limit(opts.limit ?? 60);
    ...
    // Cargar ratings en una sola query
    const ids = pubs.map((p) => p.producto_id);
    ...
    return pubs
      ...
      .map((p) => {
        ...
        const precios = (prod.productos_variantes ?? [])
