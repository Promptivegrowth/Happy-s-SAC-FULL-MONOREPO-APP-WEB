...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const { data } = await sb.from('categorias').select('nombre, seo_titulo, seo_descripcion').eq('slug', slug).single();
...
export default async function CategoriaPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const sb = await createClient();
  const { data: cat } = await sb.from('categorias').select('id, nombre, descripcion, icono, imagen_url').eq('slug', slug).single();
  if (!cat) notFound();
  ...
  return (
    <div className="container px-4 py-10">
      <header className="mb-8 flex items-center gap-4">
        ...
        <div>
          <h1 className="font-display text-4xl font-semibold">{cat.nombre}</h1>
      ...
      {(pubs ?? []).length === 0 ? (
        ...
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {pubs?.map((p) => {
            const prod = (p as unknown as { productos: { nombre: string; imagen_principal_url: string | null; productos_variantes: { precio_publico: number | null }[] } }).productos;
            ...
            return (
              <Link key={p.producto_id} href={`/productos/${p.slug}`} className="group">
                <Card className="overflow-hidden transition hover:-translate-y-1 hover:shadow-glow">
                  ...
                  <div className="p-4">
                    <h3 className="line-clamp-2 font-medium">{p.titulo_web ?? prod.nombre}</h3>
