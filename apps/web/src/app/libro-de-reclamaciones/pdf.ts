...
export async function generarReclamoPdf(r: ReclamoConfirmacion): Promise<void> {
  ...
  function bloque(t: string, y: number): number {
    doc.setFontSize(10);
    doc.setFont('helvetica', 'bold');
  ...
  let y = lastY + 6;
  y = bloque('1. IDENTIFICACIÓN DEL CONSUMIDOR', y);
  autoTable(doc, {
    startY: y,
    theme: 'grid',
    ...
    body: [
      ...
      ['Dirección', r.cliente_direccion ?? '—', 'Ubigeo', r.cliente_ubigeo ?? '—'],
      [
        'Menor de edad',
  ...
  y = lastY + 6;
  y = bloque('2. IDENTIFICACIÓN DEL BIEN CONTRATADO', y);
  autoTable(doc, {
    startY: y,
    theme: 'grid',
    styles: { fontSize: 9, cellPadding: 1.5 },
  ...
  autoTable(doc, {
    ...
    body: [
      ...
      ['Pedido del consumidor', r.pedido_consumidor ?? '—'],
    ],
    columnStyles: {
      0: { cellWidth: 30, fontStyle: 'bold', fillColor: [248, 250, 252] },
      1: { cellWidth: 'auto' },
