...
export default async function CatalogoPage({ searchParams }: { searchParams: Promise<{ q?: string; cat?: string }> }) {
  const sp = await searchParams;
  const sb = await createClient();
  let q = sb.from('productos_publicacion')
    .select('producto_id, slug, titulo_web, precio_oferta, productos!inner(id, nombre, imagen_principal_url, categoria_id, productos_variantes(id, talla, precio_publico))')
    .eq('publicado', true)
    .order('orden_web')
    .limit(60);
  if (sp.q) q = q.ilike('titulo_web', `%${sp.q}%`);
            ...
            return (
              <Link key={p.producto_id} href={`/productos/${p.slug}`} className="group">
                <Card className="overflow-hidden transition hover:-translate-y-1 hover:shadow-glow">
                  ...
                  <div className="p-4">
                    <h3 className="line-clamp-2 font-medium leading-tight">{p.titulo_web ?? prod.nombre}</h3>
